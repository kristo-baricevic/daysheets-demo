      ...
      <div class="modal">
        <div class="modalHead">
          <div class="headLeft">
            ...
            <div class="muted">
              {{ tab === "lodging" ? "Find a hotel to add to your itinerary" : "Select the guests staying at this hotel" }}
            </div>
          </div>
  
          <div class="headRight">
            <div class="tabs">
              <button type="button" class="tabBtn" :class="{ active: tab === 'lodging' }" @click="tab = 'lodging'">
        ...
        <div class="body">
          ...
          <div v-else class="pane">
            ...
            <div class="guestSearchRow">
              <input class="input" v-model="guestQuery" placeholder="Search to add personnel to the rooming list" />
        ...
        <div class="foot">
          ...
          <button v-if="tab === 'lodging'" class="btn primary" type="button" :disabled="!canGoGuests" @click="tab = 'guests'">
            Next
          </button>
...
const loadFromExisting = (l: EditLodging | null | undefined) => {
  ...
  tab.value = "lodging";
  console.log("loadFromExisting 2",selectedHotel )
      ...
      hotelTimer = setTimeout(async () => {
        try {
          ...
          hotels.value = (res ?? []).map((x: any) => ({
            ...
            placeId: x.placeId,
            source: x.source,
            addressLine: [x.address1, [x.city, x.state].filter(Boolean).join(", "), x.postal].filter(Boolean).join(" "),
          }));
          highlightedHotelIdx.value = 0;
        } finally {
          hotelsLoading.value = false;
